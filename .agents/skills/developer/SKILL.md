...
- Implement the requested components. Ensure:
  ...
  - **Reactive Notifier State Placement**: Always access and read reactive state variables (e.g., `widget.appState.bondRecords` or lists of records) *inside* the builder callback of a `ListenableBuilder` or other reactive widget (not in the outer build method block) to ensure Flutter registers the dependency and correctly rebuilds the widget when `notifyListeners()` is called.
  - **Shared Design Tokens Validation**: Verify that any typography (`AppTypography`) or color (`AppColors`) styles used exist in the design system to avoid compilation errors.
  - **Admin Telemetry & Manual Sync Mapping**: When integrating a new dataset, ensure its ID constant is registered in the client-side telemetry notifier (`telemetry_notifier.dart`) and mapped to the correct Cloud Function in `triggerManualSync`. Additionally, register mock metadata/records in `initAdminMetadataListener` and `initDirectoryListener` to prevent null-reference issues during local manual sync runs.
