...
class BankAtmsNotifier extends ChangeNotifier {
  ...
  List<BankAtmRecordModel> _atmRecords = [];
  bool _isLoadingAtms = true;
  StreamSubscription<QuerySnapshot>? _atmsSubscription;
  ...
  List<BankAtmRecordModel> get atmRecords => _atmRecords;

  /// Checks if Bank ATMs query is loading.
  bool get isLoadingAtms => _isLoadingAtms;
  ...
  void initBankAtmsListener() {
    ...
    if (testFirestoreStream != null) {
      ...
      _atmsSubscription = testFirestoreStream!.listen(
        ...
        onError: (Object err) {
          _isLoadingAtms = false;
          notifyListeners();
          AppLogger.error('Firestore Bank ATMs collection listener error', err);
        },
      );
      return;
    }
    ...
    try {
      _atmsSubscription = (testFirestore ?? FirebaseFirestore.instance)
          ...
          .listen(
            ...
            onError: (Object err) {
              ...
              notifyListeners();
              AppLogger.error(
                'Firestore Bank ATMs collection listener error',
                err,
              );
            },
          );
      ...
      notifyListeners();
      AppLogger.error(
        'Failed to initialize Bank ATMs listener in BankAtmsNotifier',
        e,
      );
    }
  }
